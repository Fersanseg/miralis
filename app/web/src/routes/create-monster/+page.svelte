...
<div class="grid grid-cols-[1fr_min-content] max-w-7xl bg-base-200 rounded-xl p-10">
	<form action="?/createMonster" method="POST" enctype="multipart/form-data" class="flex flex-col">
		...
		<textarea name="description" cols="30" rows="10" class="mb-5" />

		<InputLabel label="Skills:" inputType="text" inputName="skills" />
		<InputLabel
			label="Parts value (empty if no specific value needed):"
			inputType="text"
			inputName="partsValue"
		/>
		<InputLabel
			label="Parts bulk (empty if no specific value needed):"
			inputType="text"
			inputName="partsBulk"
		/>
		...
		<div>
			...
			<input type="checkbox" name="hidden" class="w-4 h-4" />
		</div>
